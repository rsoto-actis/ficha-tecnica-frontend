...
export class FavouritesComponent {
  ...
  @ViewChild('piramidalEdit') piramidalEdit! : ElementRef;
  ...
  toggleLiveDemo( ts : any ) {
    ...
    this.getExtraFichaTecnicaData(ts.id)
  }
  ...
  public getExtraFichaTecnicaData( id : number ){
    
    this.proyectoService
      .getExtraFichaTecnicaData(id)
      .pipe(first())
      .subscribe(
        ( result: any ) => {
          ...
          if ( result != null ){
            this.extraProjectData = result;
            this.piramidalEdit.nativeElement.checked = result.piramidal;

          }
          else{
            this.extraProjectData = {
              id              : 1,
              proyecto_id     : this.choosenProject.id,
              piramidal       : false,
              created         : new Date(),
              modified        : new Date(),
              favoritos       : false,
            ...
            this.piramidalEdit.nativeElement.checked = false;
          }

          console.log(this.extraProjectData)
        },
        ( error : any) => {
          console.log(error)
        }
  ...
  public confirmChangeFav(){    
    ...
    if ( index >= 0 ){
      ...
      if ( this.proyectFavAux.favoritos == 0 ){
        this.proyectFavAux.favoritos == 1;
        ...
        this.projects[index].favoritos     = 0;
      }
    }
    this.favFlag = false;
